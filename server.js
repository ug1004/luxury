...
try {
        ...
        const network = await gateway.getNetwork('ulsanchaneel');
        const contract = network.getContract('goods');

        await contract.submitTransaction('Goods_register', serial, maker, email, info);
...
app.get('/goods/history', async (req, res) => { // (TO DO) url 변경
    ...
    try {
        ...
        console.log('/goods/history-get-' + cert + '-' + carid);
